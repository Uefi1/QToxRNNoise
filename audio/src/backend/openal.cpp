...
bool OpenAL::initInput(const QString& deviceName, uint32_t channels)
{
    ...
    noiseSuppressors.clear();
    noiseSuppressors.reserve(inputChannels);
    for (int i = 0; i < inputChannels; ++i) {
        noiseSuppressors.push_back(std::make_unique<NoiseSuppressor>());
    }
...
void OpenAL::doInput()
{
    ...
    // Denoise each channel in place, before gain/threshold, so RNNoise sees
    // the raw mic signal rather than an already-boosted one.
    if (settings.getEnableNoiseSuppression()) {
        for (int ch = 0; ch < inputChannels && ch < static_cast<int>(noiseSuppressors.size());
             ++ch) {
            noiseSuppressors[static_cast<size_t>(ch)]->processChannel(
                inputBuffer + ch, static_cast<int>(AUDIO_FRAME_SAMPLE_COUNT_PER_CHANNEL),
                inputChannels);
        }
    }
