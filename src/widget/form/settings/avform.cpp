    ...
    , camera(camera_)
{
    ...
    cbEnableNoiseSuppression->setChecked(audioSettings_->getEnableNoiseSuppression());
    cbEnableNoiseSuppression->setToolTip(
        tr("Reduces steady background noise (fans, hiss, hum) picked up by your microphone."));
...
void AVForm::on_cbEnableNoiseSuppression_stateChanged()
{
    audioSettings->setEnableNoiseSuppression(cbEnableNoiseSuppression->isChecked());
}
